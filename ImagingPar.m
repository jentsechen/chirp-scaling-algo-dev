classdef ImagingPar
    ...
    methods
        ...
        function point_target_echo_signal = point_target_echo_signal(obj)
            point_target_echo_signal = zeros(length(obj.azimuth_time_axis_sec), length(obj.range_time_axis_sec));
            for i = 1:length(obj.azimuth_time_axis_sec)
        ...
        function plot_point_target_echo_signal(obj, point_target_echo_signal, x_axis_mode, y_axis_mode)
            if x_axis_mode == AxisMode.TimeSecond
                x_axis = obj.range_time_axis_sec * 1e6;
            end
            if y_axis_mode == AxisMode.TimeSecond
                y_axis = obj.range_time_axis_sec * 1e6;
            end
            figure;
    ...
    methods (Access = private)
        ...
        function azimuth_time_axis_sec = gen_azimuth_time_axis_sec(obj)
            azimuth_time_axis_sec_len = floor(obj.synthetic_aperture_time_sec * obj.sig_par.pulse_rep_freq_hz / 2) * 2;
            azimuth_time_axis_sec = (-azimuth_time_axis_sec_len/2 : azimuth_time_axis_sec_len/2-1)' / obj.sig_par.pulse_rep_freq_hz;
        end
        ...
        function range_window = range_window(obj, round_trip_time_sec)
            range_window = ((obj.range_time_axis_sec-round_trip_time_sec)>-obj.sig_par.pulse_width_sec/2) & ...
                           ((obj.range_time_axis_sec-round_trip_time_sec)<obj.sig_par.pulse_width_sec/2);
        end
